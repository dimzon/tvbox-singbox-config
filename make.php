...
function makeJson($debug){
    ...
    if($debug){
        ...
        file_put_contents("tv-config-debug.json", json_encode($template, JSON_PRETTY_PRINT));
    } else {
        $remoteRuleSet = array("type"=>"remote", "tag"=>"tv", "update_interval"=>"2h",
        "download_detour"=> "proxy", "url"=>"https://raw.githubusercontent.com/dimzon/tvbox-singbox-config/refs/heads/main/tv-ruleset.srs", "format"=>"binary");
        $template->route->rule_set[]=$remoteRuleSet;
        file_put_contents("tv-config-release.json", json_encode($template, JSON_PRETTY_PRINT));
    }
}
